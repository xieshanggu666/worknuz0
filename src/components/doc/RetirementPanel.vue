...
const canInitiate = computed(() =>
  isOwner.value && !active.value && !open.value
)
...
<template>
  <section class="retire card">
    ...
    <div v-else-if="open" class="rt-banner pending">
      ...
      <div class="rt-meta">{{ userName(open.initiatedBy) }} 于 {{ formatFull(open.createdAt) }} 发起</div>
...
.rt-meta { font-size: 12px; color: var(--text-3); }

...
async function decide(r, decision) {
  ...
  busyId.value = r.id
  try {
    const res = await retirementStore.decideRetirement(r.id, decision, (noteMap.value[r.id] || '').trim(), auth.user)
    if (res.status === 'ok') {
      if (res.approved) {
        alert('退役已生效：旧文档已停止搜索与问答引用，共享链接已撤销，缺口工单答案来源已改挂替代文档。')
      }
...
async function cancel(r) {
  if (!confirm('确定撤销本次退役申请？旧文档保持原状。')) return
...
async function revoke(r) {
  const note = (noteMap.value[r.id] || '').trim()
  if (!confirm('确定撤销退役？将恢复旧文档搜索/问答引用、恢复共享链接、答案来源回挂旧文档。')) return
  busyId.value = r.id
  try {
    const res = await retirementStore.revokeRetirement(r.id, note, auth.user)
    if (res.status === 'ok') {
...
<template>
  <div class="rt-page">
    <header class="head">
      <h2>🗄 知识退役替代</h2>
      <p class="sub">
        负责人发起文档退役并指定替代文档，管理员审批后停止旧文档的搜索与问答引用、撤销其共享链接，
        并把已解决缺口工单的答案来源改挂替代文档；替代文档无权限时可申请访问。退役可撤销，共享链接与答案来源同步恢复，全程留痕。
      </p>
      <div class="tabs">
    ...
    <div v-if="!list.length" class="empty card">
      <div class="ico">🗄</div>
      {{ tab === 'approve' ? '暂无待审批的退役申请' : tab === 'mine' ? '你还没有发起过文档退役（可在文档详情页发起）' : '暂无退役记录' }}
    </div>

    <div v-else class="rt-list">
      <div v-for="r in list" :key="r.id" class="rt card">
        <div class="rt-top">
          <div class="rt-flow">
          ...
          <div class="rt-side">
            ...
            <span class="rt-time">{{ formatDate(r.createdAt) }}</span>
          </div>
        </div>
        ...
        <div v-if="canCancelRetirement(r, auth.user?.id, auth.user?.role)" class="row-actions">
          <button class="btn sm ghost" @click="cancel(r)">撤销退役申请</button>
        </div>

        <!-- 发起人/管理员撤销已生效退役 -->
        <div v-if="canRevokeRetirement(r, auth.user?.id, auth.user?.role)" class="decide-box">
          <input v-model="noteMap[r.id]" class="note-in" placeholder="撤销退役说明（可选，将写入记录）" />
          <button class="btn sm restore-solid" :disabled="busyId === r.id" @click="revoke(r)">↩ 撤销退役并恢复</button>
        </div>
...
.sub { color: var(--text-2); font-size: 13px; margin: 0 0 14px; line-height: 1.7; }
...
.rt-list { display: flex; flex-direction: column; gap: 12px; margin-top: 16px; }
.rt { padding: 16px 20px; }
.rt-top { display: flex; justify-content: space-between; gap: 14px; }
.rt-flow { display: flex; align-items: center; gap: 10px; font-size: 15px; font-weight: 700; flex-wrap: wrap; min-width: 0; }
...
.rt-info { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; margin-top: 12px; font-size: 13px; }
